...
export async function handleWebCommand(host: SlashCommandHost): Promise<void> {
      ...
      options: [
        {
          ...
          label: 'Continue',
          description:
            'Start the Landa server (background daemon if needed), open this session in your default browser, and exit the terminal UI.',
        },
        {
          value: WEB_CANCEL,
          label: 'Cancel',
  ...
  host.showStatus('Starting Landa server and opening web UI…');
  let origin: string;
  try {
